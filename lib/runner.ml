...
let run_no_vg_exact (program_name : string) args std_input : (string, string) result =
  let rstdout, rstdout_name, rstderr, rstderr_name, rstdin = make_tmpfiles "run" std_input in
  let ran_pid =
    Unix.create_process (program_name ^ ".run")
      (Array.of_list ([program_name ^ ".run"] @ args))
      rstdin rstdout rstdout
  in
  let _, status = waitpid [] ran_pid in
  let result =
    match status with
    | WEXITED 0 -> Ok (string_of_file rstdout_name)
    | WEXITED n -> Ok (sprintf "Error %d: %s" n (string_of_file rstdout_name))
    | WSIGNALED n -> Error (sprintf "Signalled with %d while running %s." n program_name)
    | WSTOPPED n -> Error (sprintf "Stopped with signal %d while running %s." n program_name)
  in
  List.iter close [rstdout; rstderr; rstdin];
...
let run p out runner no_builtins dont_elim_dead args std_input alloc_strat =
  let maybe_asm_string = compile_to_string ~no_builtins ~dont_elim_dead alloc_strat (Ok (p, [])) in
...
let run_anf p out runner args std_input =
  let maybe_asm_string =
    try Ok (Compile.compile_prog p) with
...
let read_options filename : compile_opts =
  ...
  let alloc_strat =
    ...
    | Some s ->
        raise
          (InternalCompilerError (sprintf "'%s' is not a valid allocation strategy, use naive" s))
  in
  { valgrind= List.mem "valgrind" opts;
    no_builtins= List.mem "no_builtins" opts;
    dont_elim_dead= List.mem "dont_elim_dead" opts;
    heap_size;
    alloc_strat }
;;
...
let test_run
  ...
  let result =
    try
      ...
      run program full_outfile run_no_vg no_builtins dont_elim_dead args std_input alloc_strat
...
let test_run_valgrind
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_vg no_builtins dont_elim_dead args std_input alloc_strat
    with err -> Error (Printexc.to_string err ^ "\n" ^ Printexc.get_backtrace ())
  in
  assert_equal (Ok (expected ^ "\n")) result ~cmp ~printer:result_printer
;;
...
let test_err
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile runner no_builtins dont_elim_dead args std_input alloc_strat
    with err -> Error (Printexc.to_string err ^ "\n" ^ Printexc.get_backtrace ())
  in
...
let test_exact
  ...
  let result =
    try
      let program = parse_string outfile program_str in
      run program full_outfile run_no_vg_exact no_builtins dont_elim_dead args std_input alloc_strat
    with err -> Error (Printexc.to_string err ^ "\n" ^ Printexc.get_backtrace ())
  in
  assert_equal (Ok msg) result ~printer:result_printer ~cmp
;;
...
let test_does_run filename test_ctxt =
  ...
  let progfile = sprintf "test/input/do_pass/%s.parr" filename in
...
let test_does_err filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "test/input/do_err/%s.parr" filename in
...
let test_doesnt_run filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "test/input/dont_pass/%s.parr" filename in
  ...
  let result =
    try
      ...
      run program full_outfile runner opts.no_builtins opts.dont_elim_dead args input alloc_strat
    with err -> Error (Printexc.to_string err ^ "\n" ^ Printexc.get_backtrace ())
  in
  match result with
  | Ok unexpected ->
...
let test_doesnt_err filename _ =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "test/input/dont_err/%s.parr" filename in
...
let test_exact_dir filename test_ctxt =
  let filename = Filename.remove_extension filename in
  let progfile = sprintf "test/input/exact/%s.parr" filename in
  let argsfile = sprintf "test/input/exact/%s.args" filename in
  let outfile = sprintf "test/input/exact/%s.out" filename in
  let infile = sprintf "test/input/exact/%s.in" filename in
  let opts = read_options (sprintf "test/input/exact/%s.options" filename) in
  let prog = string_of_file progfile in
  let args = parse_args argsfile opts in
  let output =
    if Sys.file_exists outfile then
      string_of_file outfile
    else
      ""
  in
  let input =
  ...
  in
  let alloc_strat = opts.alloc_strat in
  test_exact ~no_builtins:opts.no_builtins ~args ~std_input:input alloc_strat prog
    ("exact/" ^ filename) output test_ctxt
;;
...
let input_file_test_suite () =
  ...
  "input-file-suite"
  >::: [ "do_pass"
         >::: List.map (fun f -> f >:: test_does_run f) (safe_readdir "test/input/do_pass" ".parr");
         "do_err"
         >::: List.map (fun f -> f >:: test_does_err f) (safe_readdir "test/input/do_err" ".parr");
         "dont_pass"
         >::: List.map
                (fun f -> f >:: test_doesnt_run f)
                (safe_readdir "test/input/dont_pass" ".parr");
         "dont_err"
         >::: List.map
                (fun f -> f >:: test_doesnt_err f)
                (safe_readdir "test/input/dont_err" ".parr");
         "exact"
         >::: List.map (fun f -> f >:: test_exact_dir f) (safe_readdir "test/input/exact" ".parr")
       ]
;;
